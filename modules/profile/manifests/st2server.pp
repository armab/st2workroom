class profile::st2server {
  ...
  $_disable_admin_cli_credentials = hiera('st2::disable_admin_cli_credentials', false)
  ...
  if ! $_installer_run {
    include ::st2::stanley
    $_stanley_user = $::st2::stanley::username

    ## Add CLI settings for the created 'admin' user, but require them to
    ## auth in some way via ST2_AUTH_TOKEN. This is a workaround
    ## to provide the default user with information on where the
    ## SSL endpoints are for st2api/st2auth.
    ##
    ## This block includes 'disable_credentials' to prevent any false
    ## autogenerated credentials from being generated.
    if $_disable_admin_cli_credentials {
      $_client_settings_username = undef
      $_client_settings_password = undef
    } else {
      $_client_settings_username = $_root_cli_username
      $_client_settings_password = $_root_cli_password
    }
    ::st2::client::settings { $_stanley_user:
      auth                => true,
      api_url             => $_api_url,
      auth_url            => $_auth_url,
      cache_token         => false,
      disable_credentials => $_disable_admin_cli_credentials,
      username            => $_client_settings_username,
      password            => $_client_settings_password,
      debug               => false,
    }
  }
